...
FILE_VOCABOLARIO = os.path.join(DATA_DIR, "vocabolario.txt")
FILE_TOLLERANZA = os.path.join(DATA_DIR, "tolleranza.txt")
FILE_CRAWL_ATTUALE = os.path.join(DATA_DIR, "crawl_attuale.txt")  # quale crawl stiamo scansionando ora
FILE_STATO_OGGI = os.path.join(DATA_DIR, "stato_oggi.txt")        # riepilogo per il workflow (email, ecc.)
...
def scarica_lista_path(crawl_id):
    ...
    r = richiesta_con_retry(url_lista)
    testo = gzip.decompress(r.content).decode("utf-8")
    paths = [BASE + riga.strip() for riga in testo.splitlines() if riga.strip()]
    return paths


def richiesta_con_retry(url, tentativi=5, attesa_iniziale=10, **kwargs):
    """GET con alcuni tentativi automatici: assorbe blip temporanei di rete/server
    (es. connessione chiusa senza risposta) senza far crashare l'intero script."""
    ultimo_errore = None
    for tentativo in range(1, tentativi + 1):
        try:
            ...
            ultimo_errore = e
            print(f"[Tentativo {tentativo}/{tentativi}] richiesta fallita per {url}: {e}")
            if tentativo < tentativi:
                attesa = attesa_iniziale * tentativo
                print(f"Riprovo tra {attesa}s...")
                time.sleep(attesa)
    raise ultimo_errore
...
def carica_lista_crawl():
    ...
    r = richiesta_con_retry("https://index.commoncrawl.org/collinfo.json")
    dati = r.json()
...
if __name__ == "__main__":
    ...
    FILE_CHECKPOINT = os.path.join(DATA_DIR, f"checkpoint_{CRAWL_ID}.txt")
    FILE_RISULTATI = os.path.join(DATA_DIR, f"risultati_{OGGI}_{CRAWL_ID}.xlsx")
